...
export const CommentItem = ({ data }) => {
  ...
  const handleClickLoadNestedComments = () => {
    const markup = <CommentsList data={commentData.kids} />;
  ...
  return commentData.deleted ? (
    ...
    <li style={commentsItem}>
      ...
      {commentData.kids && <p>{commentData.kids.length} nested comment('s)</p>}
      <address>by: {isLoadingData ? 'Loading...' : commentData.by}</address>
      <p>
        Date:{' '}
        <time>
          {isLoadingData ? 'Loading...' : secToString(commentData.time)}
        </time>
      </p>
      <h4>Comment text:</h4>
      {isLoadingData ? (
